...
void PollerEpoll::addChannel(Channel *ch) {
  ...
  ev.events = ch->events();
  ev.data.ptr = ch;
  trace("adding channel %lld fd %d events %d epoll %d", (long long) ch->id(),
        ch->fd(), ev.events, fd_);
...
void PollerEpoll::updateChannel(Channel *ch) {
  ...
  trace("modifying channel %lld fd %d events read %d write %d epoll %d",
        (long long) ch->id(), ch->fd(), ev.events & POLLIN, ev.events & POLLOUT,
        fd_);
...
void PollerEpoll::removeChannel(Channel *ch) {
  trace("deleting channel %lld fd %d epoll %d", (long long) ch->id(), ch->fd(),
        fd_);
...
void PollerEpoll::loop_once(int waitMs) {
  ...
  trace("epoll wait %d return %d errno %d used %lld millsecond", waitMs,
        lastActive_, errno, (long long) used);
  ...
  while (--lastActive_ >= 0) {
    ...
    if (ch) {
      if (events & (kReadEvent | POLLERR)) {
        ...
        trace("channel %lld fd %d handle write", (long long) ch->id(),
              ch->fd());
        ch->handleWrite();
      } else {
...
void PollerKqueue::addChannel(Channel *ch) {
  ...
  trace("adding channel %lld fd %d events read %d write %d  epoll %d",
        (long long) ch->id(), ch->fd(), ch->events() & POLLIN,
...
void PollerKqueue::loop_once(int waitMs) {
  ...
  trace("kevent wait %d return %d errno %d used %lld millsecond", waitMs,
        lastActive_, errno, util::timeMilli() - ticks);
  fatalif(lastActive_ == -1 && errno != EINTR, "kevent return error %d %s",
          errno, strerror(errno));
  while (--lastActive_ >= 0) {
    int i = lastActive_;
