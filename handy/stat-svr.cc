...
static string query_link(const string &path) {
  return util::format("<a href=\"/?stat=%s\">%s</a>", path.c_str(),
                      path.c_str());
}
...
StatServer::StatServer(EventBase *base) : server_(base) {
  server_.onDefault([this](const HttpConnPtr &con) {
    ...
    if (req.uri == "/") {
      ...
      buf.append("</table>\n");
      if (resp.body.size()) {
        buf.append(util::format("<br/>SubQuery %s:<br/> %s", query.c_str(),
                                resp.body.c_str()));
      }
...
void StatServer::onRequest(StatType type,
                           const string &key,
                           const string &desc,
                           const StatCallBack &cb) {
  if (type == STATE) {
...
void StatServer::onRequest(StatType type,
                           const string &key,
                           const string &desc,
                           const InfoCallBack &cb) {
  onRequest(type, key, desc,
            [cb](const HttpRequest &, HttpResponse &r) { r.body = cb(); });
}

void StatServer::onPageFile(const string &page,
                            const string &desc,
                            const string &file) {
  return onRequest(
      PAGE, page, desc, [file](const HttpRequest &req, HttpResponse &resp) {
