...
struct StatServer : private noncopyable {
  ...
  //用于展示一个简单的state
  void onState(const string &state,
               const string &desc,
               const InfoCallBack &cb) {
  ...
  //用于展示一个页面
  void onPage(const string &page, const string &desc, const InfoCallBack &cb) {
    onRequest(PAGE, page, desc, cb);
  }
  void onPageFile(const string &page, const string &desc, const string &file);
  //用于发送一个命令
  void onCmd(const string &cmd, const string &desc, const InfoCallBack &cb) {
