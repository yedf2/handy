...
void EventsImp::loop() {
  ...
  idleConns_.clear();
  for (auto recon :
       reconnectConns_) {  //重连的连接无法通过channel清理，因此单独清理
    recon->cleanup(recon);
  }
  loop_once(0);
}
...
IdleId EventsImp::registerIdle(int idle,
...
TcpConn::~TcpConn() {
  trace("tcp destroyed %s - %s", local_.toString().c_str(),
...
void TcpConn::addIdleCB(int idle, const TcpCallBack &cb) {
  if (channel_) {
    idleIds_.push_back(
        getBase()->imp_->registerIdle(idle, shared_from_this(), cb));
  }
}
...
void TcpConn::reconnect() {
  ...
  long long interval =
      reconnectInterval_ - (util::timeMilli() - connectedTime_);
  interval = interval > 0 ? interval : 0;
  info("reconnect interval: %d will reconnect after %lld ms",
