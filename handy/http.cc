    ...
    *line1 = req.eatLine();
    while (req.size()) {
      ...
      if (k.size() && ln.size() && k.back() == ':') {
        ...
        error("bad http line: %.*s %.*s", (int) k.size(), k.data(),
              (int) ln.size(), ln.data());
        return Error;
      }
    }
...
HttpMsg::Result HttpRequest::tryDecode(Slice buf, bool copyBody) {
  ...
  if (ln1.size()) {
    ...
    if (query_uri.size() == 0 || query_uri[0] != '/') {
      error("query uri '%.*s' should begin with /", (int) query_uri.size(),
            query_uri.data());
      return Error;
    }
...
void HttpConnPtr::handleRead(const HttpCallBack &cb) const {
  if (!tcp->isClient()) {  // server
    ...
    if (r == HttpMsg::Continue100) {
      ...
      info("http request: %s %s %s", req.method.c_str(), req.query_uri.c_str(),
           req.version.c_str());
      trace("http request:\n%.*s", (int) tcp->input_.size(),
            tcp->input_.data());
      cb(*this);
    }
  } else {
