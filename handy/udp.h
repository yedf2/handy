...
typedef std::function<void(const UdpServerPtr &, Buffer, Ip4Addr)>
    UdpSvrCallBack;
const int kUdpPacketSize = 4096;
// Udp服务器
struct UdpServer : public std::enable_shared_from_this<UdpServer>,
                   private noncopyable {
  ...
  static UdpServerPtr startServer(EventBases *bases,
...
// Udp连接，使用引用计数
struct UdpConn : public std::enable_shared_from_this<UdpConn>,
                 private noncopyable {
  // Udp构造函数，实际可用的连接应当通过createConnection创建
  UdpConn(){};
  virtual ~UdpConn() { close(); };
  static UdpConnPtr createConnection(EventBase *base,
                                     const std::string &host,
                                     short port);
...
typedef std::function<
    std::string(const UdpServerPtr &, const std::string &, Ip4Addr)>
    RetMsgUdpCallBack;
//半同步半异步服务器
struct HSHAU;
typedef std::shared_ptr<HSHAU> HSHAUPtr;
struct HSHAU {
  static HSHAUPtr startServer(EventBase *base,
                              const std::string &host,
                              short port,
                              int threads);
