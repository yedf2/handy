...
int UdpServer::bind(const std::string &host, short port, bool reusePort) {
  ...
  channel_->onRead([this] {
    ...
    buf.addSize(rn);
    trace("udp %d recv %ld bytes from %s", fd, rn,
          Ip4Addr(raddr).toString().data());
    this->msgcb_(shared_from_this(), buf, raddr);
  });
  return 0;
}

UdpServerPtr UdpServer::startServer(EventBases *bases,
...
void UdpServer::sendTo(const char *buf, size_t len, Ip4Addr addr) {
  if (!channel_ || channel_->fd() < 0) {
    warn("udp sending %lu bytes to %s after channel closed", len,
         addr.toString().data());
    return;
  }
  int fd = channel_->fd();
  int wn =
      ::sendto(fd, buf, len, 0, (sockaddr *) &addr.getAddr(), sizeof(sockaddr));
  if (wn < 0) {
    error("udp %d sendto %s error: %d %s", fd, addr.toString().c_str(), errno,
          strerror(errno));
    return;
  }
...
UdpConnPtr UdpConn::createConnection(EventBase *base,
                                     const string &host,
                                     short port) {
...
HSHAUPtr HSHAU::startServer(EventBase *base,
                            const std::string &host,
                            short port,
                            int threads) {
