...
void TcpConn::attach(EventBase *base, int fd, Ip4Addr local, Ip4Addr peer) {
  ...
  trace("tcp constructed %s - %s fd: %d", local_.toString().c_str(),
...
void TcpConn::connect(EventBase *base,
                      const string &host,
                      short port,
                      int timeout,
                      const string &localip) {
  fatalif(state_ != State::Invalid && state_ != State::Closed &&
              state_ != State::Failed,
  ...
  if (localip.size()) {
    ...
    r = ::bind(fd, (struct sockaddr *) &addr.getAddr(),
               sizeof(struct sockaddr));
    error("bind to %s failed error %d %s", addr.toString().c_str(), errno,
          strerror(errno));
  }
  if (r == 0) {
    r = ::connect(fd, (sockaddr *) &addr.getAddr(), sizeof(sockaddr_in));
    if (r != 0 && errno != EINPROGRESS) {
      error("connect to %s error %d %s", addr.toString().c_str(), errno,
            strerror(errno));
    }
  }
...
int TcpConn::handleHandshake(const TcpConnPtr &con) {
  fatalif(state_ != Handshaking, "handleHandshaking called when state_=%d",
          state_);
  struct pollfd pfd;
  pfd.fd = channel_->fd();
...
void TcpConn::handleWrite(const TcpConnPtr &con) {
  if (state_ == State::Handshaking) {
    ...
    if (output_.empty() &&
        channel_->writeEnabled()) {  // writablecb_ may write something
...
ssize_t TcpConn::isend(const char *buf, size_t len) {
  ...
  while (len > sended) {
    ...
    trace("channel %lld fd %d write %ld bytes", (long long) channel_->id(),
          channel_->fd(), wd);
    if (wd > 0) {
      sended += wd;
      continue;
      ...
      error("write error: channel %lld fd %d wd %ld %d %s",
            (long long) channel_->id(), channel_->fd(), wd, errno,
            strerror(errno));
      break;
    }
  }
...
int TcpServer::bind(const std::string &host, short port, bool reusePort) {
  ...
  if (r) {
    close(fd);
    error("bind to %s failed %d %s", addr_.toString().c_str(), errno,
          strerror(errno));
    return errno;
  }
  r = listen(fd, 20);
...
TcpServerPtr TcpServer::startServer(EventBases *bases,
  ...
  if (r) {
    error("bind to %s:%d failed %d %s", host.c_str(), port, errno,
          strerror(errno));
  }
  return r == 0 ? p : NULL;
}
...
void TcpServer::handleAccept() {
  ...
  int cfd;
  while (lfd >= 0 &&
         (cfd = accept(lfd, (struct sockaddr *) &raddr, &rsz)) >= 0) {
...
HSHAPtr HSHA::startServer(EventBase *base,
                          const std::string &host,
                          short port,
                          int threads) {
