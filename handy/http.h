...
struct HttpMsg {
  ...
  std::string getHeader(const std::string &n) {
    return getValueFromMap_(headers, n);
  }
 ...
 protected:
  ...
  std::string getValueFromMap_(std::map<std::string, std::string> &m,
                               const std::string &n);
};
...
struct HttpConnPtr {
  ...
  HttpRequest &getRequest() const {
    return tcp->internalCtx_.context<HttpContext>().req;
  }
  HttpResponse &getResponse() const {
    return tcp->internalCtx_.context<HttpContext>().resp;
  }
