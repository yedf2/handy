...
int main(int argc, const char *argv[]) {
  if (argc < 9) {
    printf(
        "usage %s <host> <begin port> <end port> <conn count> <create seconds> "
        "<subprocesses> <hearbeat interval> <send size> <management port>\n",
        argv[0]);
    return 1;
  }
  int c = 1;
  ...
  if (pid == 0) {  // child process
    ...
    for (int k = 0; k < create_seconds * 10; k++) {
      base.runAfter(100 * k, [&] {
        ...
        for (int i = 0; i < c; i++) {
          ...
          con->onMsg(
              new LengthCodec, [&](const TcpConnPtr &con, const Slice &msg) {
          ...
          con->onState([&, i](const TcpConnPtr &con) {
            ...
            if (st == TcpConn::Connected) {
              ...
              //                            send ++;
              //                            con->sendMsg(msg);
            } else if (st == TcpConn::Failed ||
                       st == TcpConn::Closed) {  //连接出错
          ...
          [&] {
            for (int i = 0; i < heartbeat_interval * 10; i++) {
              base.runAfter(i * 100, [&, i] {
                ...
                for (size_t j = i * block;
                     j < (i + 1) * block && j < allConns.size(); j++) {
    ...
    TcpConnPtr report =
        TcpConn::createConnection(&base, "127.0.0.1", man_port, 3000);
    ...
    base.runAfter(2000,
                  [&]() {
                    report->sendMsg(util::format(
                        "%d connected: %ld retry: %ld send: %ld recved: %ld",
    ...
    base.runAfter(
        ...
        [&]() {
          for (auto &s : subs) {
            Report &r = s.second;
            printf(
                "pid: %6ld connected %6ld retry %6ld sended %6ld recved %6ld\n",
    ...
    Signal::signal(SIGCHLD, [] {
      int status = 0;
      wait(&status);
      error("wait result: status: %d is signaled: %d signal: %d", status,
